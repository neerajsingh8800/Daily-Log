...
* **Logit Masking Execution Loop**:
  ...
  2. The FSA/PDA inspects its current state $q_t$ and identifies the set of valid next tokens $V_{\mathtt{valid}}(q_t) \subseteq V$.
  3. A logit mask $M_t \in \{0, -\infty\}^{\vert{}V\vert{}}$ is generated, where $M_{t, i} = 0$ for $i \in V_{\mathtt{valid}}(q_t)$ and $-\infty$ otherwise.
...
Given raw unconstrained logit values $z_t(v)$ for each vocabulary token $v \in V$, state $q_t \in Q$, and set of valid next tokens $V_{\mathtt{valid}}(q_t) \subseteq V$:

$$P(x_t = v \mid x_{<t}, q_t) = \begin{cases} \frac{\exp(z_t(v))}{\sum_{u \in V_{\mathtt{valid}}(q_t)} \exp(z_t(u))}, & \text{if } v \in V_{\mathtt{valid}}(q_t) \\ 0, & \text{otherwise} \end{cases}$$
...
* $\delta$: Transition function $Q \times (\Sigma \cup \{\epsilon\}) \times \Gamma \to \mathcal{P}(Q \times \Gamma^*)$.

The valid token set $V_{\mathtt{valid}}(q_t, \gamma_t)$ for state $q_t$ and stack top $\gamma_t \in \Gamma$ is:

$$V_{\mathtt{valid}}(q_t, \gamma_t) = \left\{ v \in \Sigma \mid \exists (q', \alpha) \in \delta(q_t, v, \gamma_t) \right\}$$
...
| :--- | :--- | :--- | :--- | :--- |
| **Prompt Engineering (Few-Shot)** | High ($5\% - 20\%$) | $0\%$ | No | Quick prototyping |
| **JSON Mode (Soft-Retry)** | Low ($1\% - 5\%$) | High (Re-runs full forward pass) | No | Simple JSON payloads |
